...
export async function readNotification(req,res){
    try{
        let notification= await Notifications.findOne({where:{uuid:req.params.notificationId}})
        if(!notification){
            return res.status(400).send("Unable to find notification")
        }
        await Notifications.update({isRead:true},{where:{uuid:req.params.notificationId}})
